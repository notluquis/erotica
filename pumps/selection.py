"""Selection-function / completeness helpers for PUMPS member samples.
...
dependencies into ``import pumps``:
...
imported lazily, exactly like :mod:`pumps.analysis.external.asteca`, so that
...
Data model
----------
A PUMPS member table is an :class:`astropy.table.QTable` with (at least) the
columns ``ra`` and ``dec`` (deg), ``Gmag`` (Gaia G), ``parallax_error`` (mas),
and a membership ``probability`` column -- the same names used across
:mod:`pumps.analysis.structure` and :mod:`pumps.io`.
"""
...
# ---------------------------------------------------------------------------
# Lazy optional-dependency guards (mirrors pumps.analysis.external.asteca)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Small unit helper (kept local to avoid importing the heavy pumps.analysis
# package just for pumps.analysis.units.quantity_values).
# ---------------------------------------------------------------------------
def _values_in(value: Any, unit: u.UnitBase | None = None) -> np.ndarray:
    ...
    Mirrors :func:`pumps.analysis.units.quantity_values` so behaviour matches the
    rest of the codebase, without triggering the heavy ``pumps.analysis`` import.
    """
