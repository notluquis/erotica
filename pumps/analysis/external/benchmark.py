"""Head-to-head membership benchmark harness: PUMPS vs ASteCA vs pyUPMASK.
...
emit membership probabilities too. The complementary point is that PUMPS's
...
scikit-learn (all core PUMPS dependencies), so the harness is fully usable --
and unit-tested -- without any external clustering package installed.

The *runner* hooks (``run_pumps``, ``run_asteca_membership``,
``run_pyupmask_membership``) actually execute each pipeline. They import their
heavy / optional dependency **lazily**, matching ``pumps.analysis.external.
...
2. PARAMETER-NAME NORMALIZATION. ASteCA reports ``loga`` / ``dm`` / ``Av`` /
   ``met`` (as Z); PUMPS inference reports distance in pc, linear age, etc.
...
3. ``pumps.calibration`` DOES NOT EXIST YET. The calibration metrics live here
   inline. They are self-contained and should later be extracted into a
   dedicated ``pumps/calibration.py`` and imported from there.
"""
...
# ---------------------------------------------------------------------------
# Lazy optional-dependency guards (mirror pumps.analysis.external.asteca)
# ---------------------------------------------------------------------------
def _require_asteca():
    try:
        import asteca
...
class BenchmarkReport:
    ...
    # -- convenience projections (pandas is a core PUMPS dep; import lazily) --
...
def compute_parameter_recovery(
    ...
    *,
    ...
    PUMPS distance-in-pc, ...) onto that schema before calling.
    """
...
def run_pumps(
    ...
    **search_kwargs,
    ...
    """Run PUMPS's pseudo-probability pipeline and return (p_tilde, mask, seconds).

    ``p_tilde`` is PUMPS's ``probability`` column = probability_hdbscan *
    probability_times (see ``Clustering.search_pseudoprobability``). The default
    mcs sweep in PUMPS is ``range(10, 300)`` and is slow; ``min_cluster_size_samples``
    is exposed here so smoke tests can use a tiny range.

    MUST be verified end-to-end in the real env: this wires
    ``pumps.Clustering`` but has not been executed against live Gaia data here.
    """
    from pumps import Clustering  # lazy: keep module import cheap
