# Maximizing HDBSCAN for OC membership — plan for COSMIC
...
COSMIC clusters in **2D proper-motion only** (`search_pseudoprobability(columns=["pmra","pmdec"])`),
...
5. **Fix `min_samples`** `[S/I]` — COSMIC leaves it `None` (≡ min_cluster_size, coupling both knobs);
...
Crossmatch COSMIC sources to the published **H&R DR3 members** (CDS **J/A+A/673/A114**) by Gaia
`source_id`; label = H&R member (prob>0.5). Reliability diagram of COSMIC p̃ vs H&R-membership
frequency, feeding `cosmic/calibration.py` (already built + validated on synthetic; needs the full
