"""Helper utilities supporting the EROTICA data loader."""
...
def fill_masked_columns(table: QTable) -> int:
    ...
    ``erotica.preprocess._helpers.fill_missing_values`` (preprocessing).
    """
    updated = 0
        ...
        if hasattr(column, "unit"):
            ...
            if unit_key in UNIT_CORRECTIONS:
                ...
                if logger is not None:
                    logger.info(
                        "Updated units for column '%s' to %s.",
                        column_name,
                        corrected_unit,
                    )
