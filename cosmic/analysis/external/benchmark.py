"""Head-to-head membership benchmark harness: COSMIC vs ASteCA vs pyUPMASK.
...
emit membership probabilities too. The complementary point is that COSMIC's
...
scikit-learn (all core COSMIC dependencies), so the harness is fully usable --
and unit-tested -- without any external clustering package installed.

The *runner* hooks (``run_cosmic``, ``run_asteca_membership``,
``run_pyupmask_membership``) actually execute each pipeline. They import their
heavy / optional dependency **lazily**, matching ``cosmic.analysis.external.
...
2. PARAMETER-NAME NORMALIZATION. ASteCA reports ``loga`` / ``dm`` / ``Av`` /
   ``met`` (as Z); COSMIC inference reports distance in pc, linear age, etc.
...
3. ``cosmic.calibration`` DOES NOT EXIST YET. The calibration metrics live here
   inline. They are self-contained and should later be extracted into a
   dedicated ``cosmic/calibration.py`` and imported from there.
"""
...
# ---------------------------------------------------------------------------
# Lazy optional-dependency guards (mirror cosmic.analysis.external.asteca)
# ---------------------------------------------------------------------------
def _require_asteca():
    try:
        import asteca
...
class BenchmarkReport:
    ...
    # -- convenience projections (pandas is a core COSMIC dep; import lazily) --
...
def compute_parameter_recovery(
    ...
    *,
    ...
    COSMIC distance-in-pc, ...) onto that schema before calling.
    """
...
def run_cosmic(
    ...
    **search_kwargs,
    ...
    """Run COSMIC's pseudo-probability pipeline and return (p_tilde, mask, seconds).

    ``p_tilde`` is COSMIC's ``probability`` column = probability_hdbscan *
    probability_times (see ``Clustering.search_pseudoprobability``). The default
    mcs sweep in COSMIC is ``range(10, 300)`` and is slow; ``min_cluster_size_samples``
    is exposed here so smoke tests can use a tiny range.

    MUST be verified end-to-end in the real env: this wires
    ``cosmic.Clustering`` but has not been executed against live Gaia data here.
    """
    from cosmic import Clustering  # lazy: keep module import cheap
