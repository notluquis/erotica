"""Selection-function / completeness helpers for COSMIC member samples.
...
dependencies into ``import cosmic``:
...
imported lazily, exactly like :mod:`cosmic.analysis.external.asteca`, so that
...
Data model
----------
A COSMIC member table is an :class:`astropy.table.QTable` with (at least) the
columns ``ra`` and ``dec`` (deg), ``Gmag`` (Gaia G), ``parallax_error`` (mas),
and a membership ``probability`` column -- the same names used across
:mod:`cosmic.analysis.structure` and :mod:`cosmic.io`.
"""
...
# ---------------------------------------------------------------------------
# Lazy optional-dependency guards (mirrors cosmic.analysis.external.asteca)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Small unit helper (kept local to avoid importing the heavy cosmic.analysis
# package just for cosmic.analysis.units.quantity_values).
# ---------------------------------------------------------------------------
def _values_in(value: Any, unit: u.UnitBase | None = None) -> np.ndarray:
    ...
    Mirrors :func:`cosmic.analysis.units.quantity_values` so behaviour matches the
    rest of the codebase, without triggering the heavy ``cosmic.analysis`` import.
    """
