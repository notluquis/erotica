...
This module evaluates whether COSMIC's per-star membership statistic
...
:meth:`cosmic.core.clustering.Clustering.search_pseudoprobability` as the
...
``scipy`` and ``scikit-learn`` are core COSMIC dependencies and are imported at
module scope, matching :mod:`cosmic.core._estimator` and
:mod:`cosmic.analysis.structure`. ``matplotlib`` is imported lazily inside the
...
def hosmer_lemeshow(
    ...
    * ``dof = n_groups - 2`` is the convention Hosmer & Lemeshow calibrated for
      probabilities *fit* to the same sample by logistic regression. COSMIC's p̃
...
def fit_platt(
    ...
    calibrate COSMIC this is negligible, but note it for small-sample use.
    """
    p, y = _validate_pair(probabilities, labels)
    if np.unique(y).size < 2:
...
def calibration_report_from_clustering(
    ...
    **kwargs: Any,
    ...
    module never imports :mod:`cosmic.core.clustering`.
