...
---
title: 'EROTICA: an integrated, calibrated Bayesian pipeline for Gaia-era open-cluster analysis'
tags:
  - Python
  - astronomy
  - open clusters
...
`EROTICA` (Estimation, Recovery & Optimization, together with Inference, for Cluster Analysis) is a Python package for identifying and characterizing open clusters
...
probabilities validated against observed member frequencies. `EROTICA` addresses two gaps: (1) a single pipeline
...
fit, and no automated test suite. `EROTICA`'s membership statistic
...
fitting, `EROTICA` samples the binned Poisson Hess-diagram likelihood [@dolphin2002]
...
`EROTICA` therefore frames its isochrone module as a *capability* within an
...
`EROTICA` is built to *complement* this ecosystem, not to displace it. `ASteCA`
...
tasks, and `EROTICA` ships an `ASteCA` adapter (`erotica.analysis.external.asteca`)
...
verifying that `EROTICA` *agrees* with `ASteCA`, `pyUPMASK`, and the survey-scale
